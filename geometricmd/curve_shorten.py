...
from k_g_function.k_g_function import k_function
from k_g_function.k_g_function import g_function
...
##for a sequence of points in the local geodesic, compute the
def compute_Q_I_matrix(points, Q_I_matric):
    Q_I_matrixs = []
    for point in points:
        Q_I_matrixs.append(Q_I_matric(point,g_function))
...
def find_geodesic_midpoint(start_point, end_point, number_of_inner_points, dimension, mass_matrix, molecule,
...
def compute_trajectory(trajectory, local_num_nodes, tol, filename, configuration, length_function=length):
            ...
            for node_number in trajectory:
                pool.apply_async(func=find_geodesic_midpoint,
                         args=(trajectory.points[node_number - 1], trajectory.points[node_number + 1], local_num_nodes,
                               dimension, mass_matrix, molecule, node_number, length_function,),
